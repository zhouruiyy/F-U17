        ...
        case .onStatusBar:
            properties = UNoticeBarProperties(shadowOffsetY: 3,
                                              ...
                                              viewFrame: CGRect(origin: CGPoint.zero,
                                                                size: CGSize(width: UScreenWidth, height: 44.0 + UIApplication.shared.statusBarFrame.height)))
        case .onNavigationBar:
...
open class UNoticeBar: UIView {
    ...
    private func show(duration: TimeInterval, willShow: () -> Void, completed: ((_ finished: Bool) -> Void)? = nil) {
